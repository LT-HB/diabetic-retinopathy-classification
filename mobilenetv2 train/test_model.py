...
def test_mobilenet():
    ...
    train_ds, test_ds, class_names = data_load("D:\\PR\\Data\\posData\\pos2_split\\train",
                                               "D:\\PR\\Data\\posData\\pos2_split\\test", 224, 224, 16)
    # todo 加载模型，修改为你的模型名称
    model = tf.keras.models.load_model("models/mobilenet_fv_split_80.h5")
    # model.summary()
    # 测试
    ...
    show_heatmaps(title="heatmap", x_labels=class_names, y_labels=class_names, harvest=heat_maps_float,
                  save_name="results/heatmap_mobilenet_split_80.png")


# 测试cnn模型准确率
def test_cnn():
    # todo 加载数据, 修改为你自己的数据集的路径
    train_ds, test_ds, class_names = data_load("D:\\PR\\Data\\posData\\train",
                                              "D:\\PR\\Data\\posData\\val", 224, 224, 16)
    # todo 加载模型，修改为你的模型名称
